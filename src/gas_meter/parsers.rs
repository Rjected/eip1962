...
pub(crate) fn parse_g1_curve_parameters<'a>(
    bytes: &'a [u8],
) -> Result<(MaxFieldUint, usize, usize, &'a [u8]), ApiError> {
    let ((modulus, modulus_len), rest) = get_base_field_params(&bytes)?;
    let (_, rest) = split(
        rest,
        modulus_len,
        "Input is not long enough to get A parameter",
    )?;
    let (_, rest) = split(
        rest,
        modulus_len,
        "Input is not long enough to get B parameter",
    )?;
    ...
    Ok((modulus, modulus_len, order_len, rest))
}
...
/// - rest
/// eats up to the operation-specific parameters
pub(crate) fn parse_g2_curve_parameters<'a>(
    bytes: &'a [u8],
) -> Result<(MaxFieldUint, usize, usize, u8, &'a [u8]), ApiError> {
    let ((modulus, modulus_len), rest) = get_base_field_params(&bytes)?;
    let (ext_degree_encoding, rest) = split(
        &rest,
    ...
    let (_, rest) = split(
        rest,
        modulus_len,
        "Input is not long enough to get non-residue",
    )?;
    let extension_field_element_len = modulus_len * (extension_degree as usize);
    let (_, rest) = split(
        rest,
        extension_field_element_len,
        "Input is not long enough to get A parameter",
    )?;
    let (_, rest) = split(
        rest,
    ...
    Ok((modulus, modulus_len, order_len, extension_degree, rest))
}

pub(crate) fn parse_mnt_pairing_parameters<'a>(
    bytes: &'a [u8],
    ext_degree: usize,
) -> Result<
    (
...
> {
    ...
    if ate_loop_hamming > MAX_ATE_PAIRING_ATE_LOOP_COUNT_HAMMING {
        return Err(ApiError::InputError(
            "Ate pairing loop has too large hamming weight".to_owned(),
        ));
    }
    ...
    let (x_sign, rest) = split(
        ...
        "Input is not long enough to get X sign encoding",
    )?;
    let _ = match x_sign[0] {
        ...
        _ => {
            return Err(ApiError::InputError(
                "X sign is not encoded properly".to_owned(),
            ));
        }
    };
    ...
    let (exp_w0_sign, rest) = split(
        rest,
        SIGN_ENCODING_LENGTH,
        "Input is not long enough to get exp_w0 sign encoding",
    )?;
    let _ = match exp_w0_sign[0] {
        ...
        _ => {
            return Err(ApiError::InputError(
                "Exp_w0 sign is not encoded properly".to_owned(),
            ));
        }
    };
    ...
    for _ in 0..num_pairs {
        ...
        let (_, rest) = split(
            rest,
            modulus_len * 2,
            "input is not long enough to get G1 point encoding",
        )?;
        let (check_g2, rest) = decode_boolean(&rest)?;
        let (_, rest) = split(
            rest,
            modulus_len * 2 * ext_degree,
            "input is not long enough to get G2 point encoding",
        )?;
        grobal_rest = rest;
    ...
    if grobal_rest.len() != 0 {
        return Err(ApiError::InputError(
            "Input has garbage at the end for MNT4/6 pairing".to_owned(),
        ));
    }
...
pub(crate) fn parse_bls12_bn_pairing_parameters<'a>(
    bytes: &'a [u8],
    max_x_bit_limit: usize,
) -> Result<
    (
        MaxFieldUint,
        usize,
...
> {
    ...
    let (_, rest) = split(
        rest,
        modulus_len,
        "Input is not long enough to get Fp2 non-residue",
    )?;
    let (_, rest) = split(
        rest,
        modulus_len * 2,
        "Input is not long enough to get Fp6/Fp12 non-residue",
    )?;
    ...
    if grobal_rest.len() != 0 {
        return Err(ApiError::InputError(
            "Input has garbage at the end for BLS12/BN pairing".to_owned(),
        ));
    }
...
pub(crate) fn parse_hashmap_usize_u64_from_ints<'de, D>(
    deserializer: D,
) -> Result<HashMap<usize, u64>, D::Error>
where
    D: Deserializer<'de>,
{
    ...
    impl<'de> Visitor<'de> for MyVisitor {
            ...
            S: SeqAccess<'de>,
        {
            ...
            while let Some(value) = seq.next_element::<[Value; 2]>()? {
                let first = value[0]
                    .as_u64()
                    .expect(&format!("should be an integer {:?}", value[0]))
                    as usize;
                let second = value[1]
                    .as_u64()
                    .expect(&format!("should be an integer {:?}", value[1]));
