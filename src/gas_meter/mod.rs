...
pub fn meter_operation(operation: OperationType, input: &[u8]) -> Result<u64, ApiError> {
    match operation {
        OperationType::G1ADD => meter_addition_g1(&input),
        OperationType::G1MUL => meter_multiplication_g1(&input),
        OperationType::G1MULTIEXP => meter_multiexp_g1(&input),
        OperationType::G2ADD => meter_addition_g2(&input),
        OperationType::G2MUL => meter_multiplication_g2(&input),
        OperationType::G2MULTIEXP => meter_multiexp_g2(&input),
        OperationType::MNT4PAIR => meter_mnt4(&input),
        OperationType::MNT6PAIR => meter_mnt6(&input),
        OperationType::BLS12PAIR => meter_bls12(&input),
        OperationType::BNPAIR => meter_bn(&input),
    }
}
...
fn meter_addition_g1(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != modulus_len * 4 {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g1 addition metering".to_owned(),
        ));
    }
...
fn meter_addition_g2(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != modulus_len * 4 * (ext_degree as usize) {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g2 addition metering".to_owned(),
        ));
    }
    let modulus_limbs = num_limbs_for_modulus(&modulus)?;
...
fn meter_multiplication_g1(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != modulus_len * 2 + order_len {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g1 multiplication metering"
                .to_owned(),
        ));
    }
    let modulus_limbs = num_limbs_for_modulus(&modulus)?;
...
fn meter_multiplication_g2(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != modulus_len * 2 * (ext_degree as usize) + order_len {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g2 multiplication metering"
                .to_owned(),
        ));
    }
...
fn meter_multiexp_g1(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != num_pairs * (modulus_len * 2 + order_len) {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g1 multiexp metering".to_owned(),
        ));
    }
...
fn meter_multiexp_g2(input: &[u8]) -> Result<u64, ApiError> {
    ...
    if rest.len() != num_pairs * (modulus_len * 2 * (ext_degree as usize) + order_len) {
        return Err(ApiError::InputError(
            "Input is either too short or contains garbage for g2 multiexp metering".to_owned(),
        ));
    }
...
fn meter_bls12(input: &[u8]) -> Result<u64, ApiError> {
    self::meter_pairing::meter_bls12_pairing(
        input,
        &*self::meter_pairing::BLS12_PARAMS_INSTANCE,
        self::meter_pairing::BLS12_MAX_MODULUS_POWER,
    )
}

fn meter_bn(input: &[u8]) -> Result<u64, ApiError> {
    self::meter_pairing::meter_bn_pairing(
        input,
        &*self::meter_pairing::BN_PARAMS_INSTANCE,
        self::meter_pairing::BN_MAX_MODULUS_POWER,
    )
}
...
impl GasMeter {
    pub fn meter(bytes: &[u8]) -> Result<u64, ApiError> {
        let (op_type, rest) = split(
            bytes,
            OPERATION_ENCODING_LENGTH,
            "Input should be longer than operation type encoding",
        )?;
        let operation = op_type[0];
        let result = match operation {
            OPERATION_G1_ADD => meter_addition_g1(&rest),
            OPERATION_G2_ADD => meter_addition_g2(&rest),
            OPERATION_G1_MUL => meter_multiplication_g1(&rest),
            OPERATION_G2_MUL => meter_multiplication_g2(&rest),
            OPERATION_G1_MULTIEXP => meter_multiexp_g1(&rest),
            OPERATION_G2_MULTIEXP => meter_multiexp_g2(&rest),
            OPERATION_PAIRING => {
                ...
                match curve_type[0] {
                    BLS12 => meter_bls12(&rest),
                    BN => meter_bn(&rest),
                    MNT4 => meter_mnt4(&rest),
                    MNT6 => meter_mnt6(&rest),
                    _ => {
                        return Err(ApiError::InputError("Unknown curve type".to_owned()));
                    }
                }
            }
            _ => Err(ApiError::InputError("Unknown operation type".to_owned())),
        };
