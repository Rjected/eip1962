...
pub(crate) static MNT4_PARAMS_INSTANCE: Lazy<MntPairingParams> =
    Lazy::new(|| serde_json::from_str(MNT4_PARAMS_JSON).expect("must deserialize parameters"));

pub(crate) static MNT6_PARAMS_INSTANCE: Lazy<MntPairingParams> =
    Lazy::new(|| serde_json::from_str(MNT6_PARAMS_JSON).expect("must deserialize parameters"));

pub(crate) static BLS12_PARAMS_INSTANCE: Lazy<Bls12PairingParams> =
    Lazy::new(|| serde_json::from_str(BLS12_PARAMS_JSON).expect("must deserialize parameters"));

pub(crate) static BN_PARAMS_INSTANCE: Lazy<BnPairingParams> =
    Lazy::new(|| serde_json::from_str(BN_PARAMS_JSON).expect("must deserialize parameters"));

pub(crate) fn meter_mnt_pairing(
    input: &[u8],
    ...
    let g1_subgroup_check_cost_per_point = super::meter_arith::meter_multiplication(
        modulus_limbs,
        order_limbs,
        &*super::meter_arith::G1_MULTIPLICATION_PARAMS_INSTANCE,
        false,
    )?;
    let g1_subgroup_checks_cost = g1_subgroup_check_cost_per_point
        .checked_mul(num_g1_subgroup_checks as u64)
        .ok_or(ApiError::Overflow)?;

    estimate = estimate
        .checked_add(g1_subgroup_checks_cost)
        .ok_or(ApiError::Overflow)?;

    let g2_subgroup_check_cost_per_point = match ext_degree {
        2 => super::meter_arith::meter_multiplication(
            modulus_limbs,
            order_limbs,
            &*super::meter_arith::G2_EXT_2_MULTIPLICATION_PARAMS_INSTANCE,
            false,
        )?,
        3 => super::meter_arith::meter_multiplication(
            modulus_limbs,
            order_limbs,
            &*super::meter_arith::G2_EXT_3_MULTIPLICATION_PARAMS_INSTANCE,
            false,
        )?,
        _ => {
            return Err(ApiError::InputError(
                "Invalid extension degree for MNT4/6 pairing cost calculation".to_owned(),
            ));
        }
    };

    let g2_subgroup_checks_cost = g2_subgroup_check_cost_per_point
        .checked_mul(num_g2_subgroup_checks as u64)
        .ok_or(ApiError::Overflow)?;

    estimate = estimate
        .checked_add(g2_subgroup_checks_cost)
        .ok_or(ApiError::Overflow)?;
...
fn calculate_mnt_pairing_cost(
    ...
    let miller_cost = {
        ...
        miller_cost = miller_cost
            .checked_mul(num_pairs as u64)
            .ok_or(ApiError::Overflow)?;
    ...
    result = result
        .checked_add(final_exp_cost)
        .ok_or(ApiError::Overflow)?;
    result = result
        .checked_div(params.multiplier)
        .ok_or(ApiError::Overflow)?;

    Ok(result)
}

pub(crate) fn meter_bls12_pairing(
    input: &[u8],
    params: &Bls12PairingParams,
    max_power: usize,
) -> Result<u64, ApiError> {
    let (modulus, order_len, num_pairs, x, _, (num_g1_subgroup_checks, num_g2_subgroup_checks), _) =
    ...
    if x_hamming > MAX_BLS12_X_HAMMING {
        return Err(ApiError::InputError(format!(
            "Hamming weight for scalar is too large, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    let g1_subgroup_check_cost = g1_subgroup_check_cost_per_point
        .checked_mul(num_g1_subgroup_checks as u64)
        .ok_or(ApiError::Overflow)?;

    estimate = estimate
        .checked_add(g1_subgroup_check_cost)
        .ok_or(ApiError::Overflow)?;

    let g2_subgroup_check_cost_per_point = super::meter_arith::meter_multiplication(
        modulus_limbs,
        order_limbs,
        &*super::meter_arith::G2_EXT_2_MULTIPLICATION_PARAMS_INSTANCE,
        false,
    )?;
    let g2_subgroup_check_cost = g2_subgroup_check_cost_per_point
        .checked_mul(num_g2_subgroup_checks as u64)
        .ok_or(ApiError::Overflow)?;

    estimate = estimate
        .checked_add(g2_subgroup_check_cost)
        .ok_or(ApiError::Overflow)?;

    Ok(estimate)
}

pub(crate) fn meter_bn_pairing(
    input: &[u8],
...
fn eval_model(
    ...
    for (coeff, var_and_power) in coeffs_variables_and_powers.iter() {
        ...
        for (variable, power) in var_and_power.iter() {
            ...
            let variable_power_value = variable_powers
                .get(*power - 1)
                .ok_or(ApiError::MissingValue)?;
            subpart = subpart
                .checked_mul(*variable_power_value)
                .ok_or(ApiError::Overflow)?;
        }
