...
impl ArithmeticMultiexpParams for G1G2MultiexpParams {
    fn params(&self) -> (u64, (usize, u64), &HashMap<usize, u64>) {
        (
            ...
            (self.max_pairs, self.max_discount),
            &self.discounts,
        )
    }
}
...
static G2_EXT_2_MULTIPLICATION_PARAMS_JSON: &'static str =
    include_str!("g2_multiplication_ext2.json");
static G2_EXT_3_MULTIPLICATION_PARAMS_JSON: &'static str =
    include_str!("g2_multiplication_ext3.json");
...
pub(crate) static G1_MULTIPLICATION_PARAMS_INSTANCE: Lazy<G1G2MultiplicationParams> =
    Lazy::new(|| {
        serde_json::from_str(G1_MULTIPLICATION_PARAMS_JSON).expect("must deserialize parameters")
    });

pub(crate) static G2_EXT_2_MULTIPLICATION_PARAMS_INSTANCE: Lazy<G1G2MultiplicationParams> =
    Lazy::new(|| {
        serde_json::from_str(G2_EXT_2_MULTIPLICATION_PARAMS_JSON)
            .expect("must deserialize parameters")
    });

pub(crate) static G2_EXT_3_MULTIPLICATION_PARAMS_INSTANCE: Lazy<G1G2MultiplicationParams> =
    Lazy::new(|| {
        serde_json::from_str(G2_EXT_3_MULTIPLICATION_PARAMS_JSON)
            .expect("must deserialize parameters")
    });

pub(crate) static MULTIEXP_PARAMS_INSTANCE: Lazy<G1G2MultiexpParams> =
    Lazy::new(|| serde_json::from_str(MULTIEXP_PARAMS_JSON).expect("must deserialize parameters"));

pub(crate) fn meter_addition<P: ArithmeticAdditionParams>(
    modulus_limbs: usize,
    parameters: &P,
) -> Result<u64, ApiError> {
...
pub(crate) fn meter_multiplication<P: ArithmeticMultiplicationParams>(
    ...
    let one_shot = *one_shot_params
        .get(&modulus_limbs)
        .ok_or(ApiError::MissingValue)?;
    let per_limb = *per_limb_params
        .get(&modulus_limbs)
        .ok_or(ApiError::MissingValue)?;

    let mut result = per_limb
        .checked_mul(group_limbs as u64)
...
pub(crate) fn meter_multiexp<P: ArithmeticMultiplicationParams, M: ArithmeticMultiexpParams>(
    ...
    let (discount_multiplier, (max_pairs, max_discount), discount_lookup) =
        multiexp_discounts.params();
...
mod test {
    ...
    fn test_calculate_example_arithmetic_prices_bn254() {
        let addition_price =
            super::meter_addition(4, &*super::G1_ADDITION_PARAMS_INSTANCE).unwrap();
        let mul_price =
            super::meter_multiplication(4, 4, &*super::G1_MULTIPLICATION_PARAMS_INSTANCE, true)
                .unwrap();
    ...
    fn test_calculate_example_arithmetic_prices_mnt4_753() {
        let mul_price = super::meter_multiplication(
            12,
            12,
            &*super::G2_EXT_2_MULTIPLICATION_PARAMS_INSTANCE,
            true,
        )
        .unwrap();
