...
pub(crate) fn decode_group_order_with_length<'a>(
    bytes: &'a [u8],
) -> Result<((usize, MaxGroupSizeUint), &'a [u8]), ApiError> {
    use crate::public_interface::constants::*;

    let (length_encoding, rest) = split(
        bytes,
        BYTES_FOR_LENGTH_ENCODING,
        "Input is not long enough to get modulus length",
    )?;
    let length = length_encoding[0] as usize;
    if length == 0 {
        return Err(ApiError::InputError(format!(
            "Encoded group length is zero, file {}, line {}",
            file!(),
            line!()
        )));
    }
    if length > MAX_GROUP_BYTE_LEN {
        return Err(ApiError::InputError(format!(
            "Encoded group length is too large, file {}, line {}",
            file!(),
            line!()
        )));
    }
...
pub(crate) fn decode_sign_is_negative<'a>(bytes: &'a [u8]) -> Result<(bool, &'a [u8]), ApiError> {
    let (x_sign, rest) = split(
        bytes,
        SIGN_ENCODING_LENGTH,
        "Input is not long enough to get sign encoding",
    )?;
...
pub(crate) fn decode_twist_type<'a>(bytes: &'a [u8]) -> Result<(TwistType, &'a [u8]), ApiError> {
    let (twist_type_encoding, rest) = split(
        bytes,
        TWIST_TYPE_LENGTH,
        "Input is not long enough to get twist type",
    )?;
...
pub(crate) fn decode_boolean<'a>(bytes: &'a [u8]) -> Result<(bool, &'a [u8]), ApiError> {
    let (boolean_encoding, rest) = split(
        bytes,
        BOOLEAN_ENCODING_LENGTH,
        "Input is not long enough to get boolean",
    )?;
    let boolean = match boolean_encoding[0] {
        ...
        _ => {
            return Err(ApiError::InputError(
                "boolean is not encoded properly".to_owned(),
            ));
        }
    };
...
pub(crate) fn parse_modulus_and_length<'a>(
    bytes: &'a [u8],
    ...
    if length > MAX_MODULUS_BYTE_LEN {
        return Err(ApiError::InputError(format!(
            "Encoded modulus length is too large, file {}, line {}",
            file!(),
            line!()
        )));
    }
...
/// - rest
pub(crate) fn parse_modulus_and_extension_degree<'a>(
    bytes: &'a [u8],
) -> Result<(MaxFieldUint, usize, u8, &'a [u8], &'a [u8]), ApiError> {
    let ((modulus, modulus_len), rest) = get_base_field_params(&bytes)?;
    let (extension_degree, rest) = split(
        rest,
        EXTENSION_DEGREE_ENCODING_LENGTH,
        "Input is not long enough to get extension degree",
    )?;
    let extension_degree = extension_degree[0];
    if !(extension_degree == EXTENSION_DEGREE_2 || extension_degree == EXTENSION_DEGREE_3) {
        return Err(ApiError::InputError(
            "Extension degree must be 2 or 3".to_owned(),
        ));
    }

    let (nonresidue_encoding, rest) = split(
        rest,
        modulus_len,
        "Input is not long enough to Fp non-residue",
    )?;
    if rest.len() == 0 {
...
pub(crate) fn get_base_field_params(
    bytes: &[u8],
) -> Result<((MaxFieldUint, usize), &[u8]), ApiError> {
    use crate::integers::*;

    let (modulus_len, rest) = split(
        bytes,
        BYTES_FOR_LENGTH_ENCODING,
        "Input is not long enough to get modulus length",
    )?;
    ...
    if modulus_len == 0 {
        return Err(ApiError::InputError(format!(
            "Modulus is length is zero, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    let (modulus_encoding, rest) =
        split(rest, modulus_len, "Input is not long enough to get modulus")?;
    if modulus_encoding[0] == 0u8 {
        return Err(ApiError::InputError(format!(
            "In modulus encoding highest byte is zero, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    if modulus.is_zero() {
        return Err(ApiError::UnexpectedZero(
            "Modulus can not be zero".to_owned(),
        ));
    }
    if is_even(&modulus) {
        return Err(ApiError::InputError(format!(
            "Modulus is even, file {}, line {}",
            file!(),
            line!()
        )));
    }
    if modulus < MaxFieldUint::from(3u64) {
        return Err(ApiError::InputError(format!(
            "Modulus is less than 3, file {}, line {}",
            file!(),
            line!()
        )));
    }
...
pub(crate) fn num_units_for_group_order_length(order_len: usize) -> Result<usize, ApiError> {
    ...
    if limbs < NUM_GROUP_LIMBS_MIN {
        return Err(ApiError::InputError(format!(
            "Group has zero limbs, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    if limbs > NUM_GROUP_LIMBS_MAX {
        return Err(ApiError::InputError(format!(
            "Group order has too many limbs, file {}, line {}",
            file!(),
            line!()
        )));
    }
...
pub(crate) fn decode_loop_parameter_scalar_with_bit_limit<'a>(
    bytes: &'a [u8],
    bit_limit: usize,
    ...
    if length == 0 {
        return Err(ApiError::InputError(format!(
            "Loop parameter scalar has zero length, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    if first_byte == 0 {
        return Err(ApiError::InputError(format!(
            "Encoded loop parameter has zero top byte, file {}, line {}",
            file!(),
            line!()
        )));
    }
    ...
    if num_bits > bit_limit {
        return Err(ApiError::InputError(format!(
            "Number of bits for scalar is too large, file {}, line {}",
            file!(),
            line!()
        )));
    }
