...
pub fn create_fp2_extension<'a, 'b, FE: ElementRepr, F: SizedPrimeField<Repr = FE>>(
    bytes: &'b [u8],
    ...
    if extension_degree[0] != EXTENSION_DEGREE_2 {
        return Err(ApiError::UnknownParameter(
            "Extension degree expected to be 2".to_owned(),
        ));
    }
    ...
    if fp_non_residue.is_zero() {
        return Err(ApiError::UnexpectedZero(
            "Fp2 non-residue can not be zero".to_owned(),
        ));
    }
        ...
        extension_2
            ...
            .map_err(|_| {
                ApiError::UnknownParameter(
                    "Failed to calculate Frobenius coeffs for Fp2".to_owned(),
                )
            })?;
    }
...
pub fn create_fp3_extension<'a, 'b, FE: ElementRepr, F: SizedPrimeField<Repr = FE>>(
    bytes: &'b [u8],
    modulus: &MaxFieldUint,
    field_byte_len: usize,
    base_field: &'a F,
    ...
    if extension_degree[0] != EXTENSION_DEGREE_3 {
        return Err(ApiError::UnknownParameter(
            "Extension degree expected to be 3".to_owned(),
        ));
    }
    ...
    if fp_non_residue.is_zero() {
        return Err(ApiError::UnexpectedZero(
            "Fp3 non-residue can not be zero".to_owned(),
        ));
    }
    ...
    if need_frobenius {
        extension_3
            ...
            .map_err(|_| {
                ApiError::UnknownParameter(
                    "Failed to calculate Frobenius coeffs for Fp3".to_owned(),
                )
            })?;
    }
...
pub fn parse_ab_in_fp2_from_encoding<'a, 'b, FE: ElementRepr, F: SizedPrimeField<Repr = FE>>(
    encoding: &'b [u8],
    modulus_len: usize,
...
pub fn parse_ab_in_fp3_from_encoding<'a, 'b, FE: ElementRepr, F: SizedPrimeField<Repr = FE>>(
    encoding: &'b [u8],
    modulus_len: usize,
