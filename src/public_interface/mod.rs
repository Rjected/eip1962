...
impl API {
    pub fn run(bytes: &[u8]) -> Result<Vec<u8>, ApiError> {
        ...
        match op_type[0] {
            OPERATION_G1_ADD => PublicG1Api::add_points(&rest),
            OPERATION_G1_MUL => PublicG1Api::mul_point(&rest),
            OPERATION_G1_MULTIEXP => PublicG1Api::multiexp(&rest),
            OPERATION_G2_ADD => PublicG2Api::add_points(&rest),
            OPERATION_G2_MUL => PublicG2Api::mul_point(&rest),
            OPERATION_G2_MULTIEXP => PublicG2Api::multiexp(&rest),
            OPERATION_PAIRING => PublicPairingApi::pair(&rest),
            _ => {
