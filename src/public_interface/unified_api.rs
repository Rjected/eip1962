...
const_assert!(
    PREALLOCATE_FOR_RESULT_BYTES
        == crate::public_interface::constants::MAX_MODULUS_BYTE_LEN * 3 * 2
);
...
impl OperationType {
    pub fn from_u8(value: u8) -> Option<Self> {
        match value {
            G1ADD_OPERATION_RAW_VALUE => Some(OperationType::G1ADD),
            G1MUL_OPERATION_RAW_VALUE => Some(OperationType::G1MUL),
            G1MULTIEXP_OPERATION_RAW_VALUE => Some(OperationType::G1MULTIEXP),
            G2ADD_OPERATION_RAW_VALUE => Some(OperationType::G2ADD),
            G2MUL_OPERATION_RAW_VALUE => Some(OperationType::G2MUL),
            G2MULTIEXP_OPERATION_RAW_VALUE => Some(OperationType::G2MULTIEXP),
            BLS12PAIR_OPERATION_RAW_VALUE => Some(OperationType::BLS12PAIR),
            BNPAIR_OPERATION_RAW_VALUE => Some(OperationType::BNPAIR),
            MNT4PAIR_OPERATION_RAW_VALUE => Some(OperationType::MNT4PAIR),
            MNT6PAIR_OPERATION_RAW_VALUE => Some(OperationType::MNT6PAIR),
            _ => None,
        }
    }
...
pub fn perform_operation(operation: OperationType, input: &[u8]) -> Result<Vec<u8>, ApiError> {
    assert!(
        !crate::features::in_gas_metering(),
        "must be run only in production mode"
    );

    match operation {
        OperationType::G1ADD => PublicG1Api::add_points(&input),
        OperationType::G1MUL => PublicG1Api::mul_point(&input),
        OperationType::G1MULTIEXP => PublicG1Api::multiexp(&input),
        OperationType::G2ADD => PublicG2Api::add_points(&input),
        OperationType::G2MUL => PublicG2Api::mul_point(&input),
        OperationType::G2MULTIEXP => PublicG2Api::multiexp(&input),
