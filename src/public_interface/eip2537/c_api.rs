...
impl Eip2537OperationType {
    pub fn from_u8(value: u8) -> Option<Self> {
        match value {
            ...
            BLS12_MAP_FP_TO_G1_OPERATION_RAW_VALUE => Some(Eip2537OperationType::BLS12_FP_TO_G1),
            BLS12_MAP_FP2_TO_G2_OPERATION_RAW_VALUE => Some(Eip2537OperationType::BLS12_FP2_TO_G2),
            _ => None,
        }
    }
...
pub extern "C" fn eip2537_perform_operation(
    ...
    let err_out_i8: &mut [i8] =
        unsafe { std::slice::from_raw_parts_mut(err, EIP2537_PREALLOCATE_FOR_ERROR_BYTES) };
    ...
    let raw_out_i8: &mut [i8] =
        unsafe { std::slice::from_raw_parts_mut(o, EIP2537_PREALLOCATE_FOR_RESULT_BYTES) };
    let mut raw_out: &mut [u8] = unsafe { std::mem::transmute(raw_out_i8) };

    let result = match operation {
        Eip2537OperationType::BLS12_G1ADD => {
            super::EIP2537Executor::g1_add(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_G1MUL => {
            super::EIP2537Executor::g1_mul(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_G1MULTIEXP => {
            super::EIP2537Executor::g1_multiexp(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_G2ADD => {
            super::EIP2537Executor::g2_add(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_G2MUL => {
            super::EIP2537Executor::g2_mul(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_G2MULTIEXP => {
            super::EIP2537Executor::g2_multiexp(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_PAIR => {
            super::EIP2537Executor::pair(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_FP_TO_G1 => {
            super::EIP2537Executor::map_fp_to_g1(&input).map(|r| r[..].to_vec())
        }
        Eip2537OperationType::BLS12_FP2_TO_G2 => {
            super::EIP2537Executor::map_fp2_to_g2(&input).map(|r| r[..].to_vec())
        }
    };
