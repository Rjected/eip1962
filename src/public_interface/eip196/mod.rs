...
const EXPECTED_MULTIPLICATION_INPUT_LEN: usize =
    SERIALIZED_G1_POINT_BYTE_LENGTH + SCALAR_BYTE_LENGTH;
    ...
    pub fn pair<'a>(
        ...
        for _ in 0..num_pairs {
            ...
            let (g2, rest) = {
                ...
                let (g2_encoding_bytes, rest) = split(
                    rest,
                    SERIALIZED_G2_POINT_BYTE_LENGTH,
                    "not enough bytes to read G2 point",
                )?;
                let mut swapped_encoding = [0u8; SERIALIZED_G2_POINT_BYTE_LENGTH];

                // swap for x coordinate
                (&mut swapped_encoding[0..SERIALIZED_FP_BYTE_LENGTH]).copy_from_slice(
                    &g2_encoding_bytes[SERIALIZED_FP_BYTE_LENGTH..(SERIALIZED_FP_BYTE_LENGTH * 2)],
                );
                (&mut swapped_encoding[SERIALIZED_FP_BYTE_LENGTH..(SERIALIZED_FP_BYTE_LENGTH * 2)])
                    .copy_from_slice(&g2_encoding_bytes[0..SERIALIZED_FP_BYTE_LENGTH]);

                // swap for y coordinate
                (&mut swapped_encoding
                    [(SERIALIZED_FP_BYTE_LENGTH * 2)..(SERIALIZED_FP_BYTE_LENGTH * 3)])
                    .copy_from_slice(
                        &g2_encoding_bytes
                            [(SERIALIZED_FP_BYTE_LENGTH * 3)..(SERIALIZED_FP_BYTE_LENGTH * 4)],
                    );
                (&mut swapped_encoding
                    [(SERIALIZED_FP_BYTE_LENGTH * 3)..(SERIALIZED_FP_BYTE_LENGTH * 4)])
                    .copy_from_slice(
                ...
                let (g2, _) = decode_g2::decode_g2_point_from_xy_in_fp2_oversized(
                    &swapped_encoding[..],
                    SERIALIZED_FP_BYTE_LENGTH,
                    &*BN254_G2_CURVE,
                )?;
