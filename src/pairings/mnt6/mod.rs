...
mod tests {
    ...
    use crate::weierstrass::curve::{CurvePoint, WeierstrassCurve};
    use crate::weierstrass::{CurveOverFp3Parameters, CurveOverFpParameters};
    ...
    fn test_mnt6_pairing() {
        ...
        let curve_twist =
            WeierstrassCurve::new(&group_order.as_ref(), a_fp3, b_fp3, &fp3_params).unwrap();
        ...
        let x: Vec<u64> = vec![0xdc9a1b671660000, 0x46609756bec2a33f, 0x1eef55];
    ...
    fn test_ey_pendulum() {
        ...
        let b_fp = BigUint::from_str_radix("3779136", 10)
            .unwrap()
            .to_bytes_be();
        let b_fp = Fp::from_be_bytes(&base_field, &b_fp, true).unwrap();
        ...
        println!(
            "Miller loop hamming is {}",
            calculate_hamming_weight(&biguint_to_u64_vec(ate_loop_length.clone()))
        );

        println!("Exp w0 length is {} bits", w0.bits());
        println!(
            "Exp w0 hamming is {}",
            calculate_hamming_weight(&biguint_to_u64_vec(w0.clone()))
        );

        println!("Exp w1 length is {} bits", w1.bits());
        println!(
            "Exp w1 hamming is {}",
            calculate_hamming_weight(&biguint_to_u64_vec(w1.clone()))
        );
